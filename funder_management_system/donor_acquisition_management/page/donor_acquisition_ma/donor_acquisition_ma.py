...
def get_total_leads(financial_years=None):
	...
	if not years:
		...
		return {"value": 0, "label": _("Total Leads"), "fieldtype": "Int"}

	try:
		total_leads = frappe.db.count("Organisation Lead", {"financial_year_of_reachout": ["in", years]})
	...
	return {"value": total_leads, "label": _("Total Leads"), "fieldtype": "Int"}
...
def get_leads_by_thematic_area(financial_years=None):
	...
	return {"labels": labels, "datasets": [{"name": _("Leads by Thematic Area"), "values": datapoints}]}
...
def get_leads_by_sources_of_connection(financial_years=None):
	...
	return {
		"labels": labels,
		"datasets": [{"name": _("Leads by Sources of Connection"), "values": datapoints}],
	}
...
def percent_card_response(label, value):
	return {"label": label, "value": f"{value:.2f}", "fieldtype": "Percent"}
...
def grouped_chart_response(table, group_by, years, label):
	...
	return {"labels": labels, "datasets": [{"name": label, "values": datapoints}]}
