...
def setup_email_account(context, interactive=True):
    ...
    try:
        if interactive:
            ...
            click.echo(
                "Non-interactive mode not implemented. Use --interactive flag.")
    finally:
        frappe.destroy()
...
def _show_gmail_app_password_instructions():
    ...
    click.echo(click.style(
        "Gmail App Password Setup Instructions", fg="yellow", bold=True))
    click.echo("="*70)
    click.echo(
...
def _interactive_email_account_setup():
    ...
    click.echo("="*70)
    click.echo(
        "This wizard will help you configure a new email account for your site.")
    ...
    if "@" not in email_id or "." not in email_id.split("@")[1]:
        click.echo(click.style(
            "Warning: Email format seems invalid", fg="yellow"))
        if not click.confirm("Continue anyway?", default=False):
            click.echo("Setup cancelled.")
            return

    email_account_name = click.prompt(
        click.style("Account Name (e.g., 'Support', 'Sales')",
                    fg="cyan", bold=True),
        type=str,
    ...
    auth_methods = ["Basic (Username/Password)",
                    "OAuth (Recommended for Gmail/Outlook)"]
    ...
    if not service:  # Custom service
        ...
        if enable_incoming:
            ...
            email_server = click.prompt(
                click.style(
                    "Incoming server (e.g., imap.gmail.com)", fg="cyan"),
                type=str
            ).strip()

            incoming_port = click.prompt(
                click.style(
                    "Incoming port (993 for IMAP SSL, 143 for IMAP)", fg="cyan"),
                type=int,
        ...
        if enable_outgoing:
            smtp_server = click.prompt(
                click.style(
                    "Outgoing server (e.g., smtp.gmail.com)", fg="cyan"),
                type=str
            ).strip()

            smtp_port = click.prompt(
                click.style(
                    "Outgoing port (587 for TLS, 465 for SSL)", fg="cyan"),
                type=int,
                default=587
            )
    ...
    click.echo("="*70)
    click.echo(
        f"Email Address: {click.style(email_id, fg='green', bold=True)}")
    click.echo(
        f"Account Name: {click.style(email_account_name, fg='green', bold=True)}")
    ...
    if default_incoming:
        click.echo(
            f"Default Incoming: {click.style('Yes', fg='green', bold=True)}")
    if default_outgoing:
        click.echo(
            f"Default Outgoing: {click.style('Yes', fg='green', bold=True)}")

    if not service:
        if email_server:
            click.echo(
                f"Incoming Server: {click.style(email_server + ':' + str(incoming_port), fg='green')}")
        if smtp_server:
            click.echo(
                f"Outgoing Server: {click.style(smtp_server + ':' + str(smtp_port), fg='green')}")
    ...
    if click.confirm(f"\nCreate this email account?", default=True):
        try:
            ...
            if frappe.db.exists("Email Account", {"email_id": email_id}):
                click.echo(click.style(
                    f"Email account '{email_id}' already exists!", fg="red"))
                return

            if frappe.db.exists("Email Account", {"email_account_name": email_account_name}):
                click.echo(click.style(
                    f"Account name '{email_account_name}' already exists!", fg="red"))
                return
            ...
            click.echo(
                f"Account Name: {click.style(email_account_doc.name, fg='cyan', bold=True)}")
            click.echo(
            ...
            if auth_method == "OAuth":
                ...
                click.echo(
                    "2. Configure OAuth credentials with your email provider")
                click.echo(
                    "3. Authorize the connection in the Email Account form")
            ...
            if service in ["GMail", "Outlook.com"]:
                ...
                if service == "GMail":
                    click.echo(
                        "• For Gmail, consider using App Passwords if using Basic auth")
                    click.echo("• OAuth is recommended for better security")
                elif service == "Outlook.com":
                    click.echo("• Modern authentication (OAuth) is preferred")
                    click.echo(
                        "• Basic auth may require enabling legacy authentication")
    ...
    try:
        ...
        if format == "json":
            ...
            for acc in email_accounts:
                created_date = acc.creation.strftime(
                    "%Y-%m-%d") if acc.creation else "N/A"
...
def create_user(context):
    ...
    try:
        ...
        click.echo("="*50)
        click.echo(
            f"Name: {click.style(f'{first_name} {last_name}', fg='green')}")
        ...
        click.echo(
            f"User '{email}' created successfully with role '{selected_role}'")
...
def setup_email_account_cli(context, email, account_name, service, password, incoming, outgoing,
    ...
    try:
        ...
        if frappe.db.exists("Email Account", {"email_id": email}):
            click.echo(click.style(
                f"Email account '{email}' already exists!", fg="red"))
            return

        if frappe.db.exists("Email Account", {"email_account_name": account_name}):
            click.echo(click.style(
                f"Account name '{account_name}' already exists!", fg="red"))
            return
        ...
        if incoming:
            if service == "GMail":
                ...
                if not imap_server:
                    click.echo(click.style(
                        "Error: IMAP server required for custom service", fg="red"))
                    return
                doc_data.update({
        ...
        if outgoing:
            if service == "GMail":
                ...
                if not smtp_server:
                    click.echo(click.style(
                        "Error: SMTP server required for custom service", fg="red"))
                    return
                doc_data.update({
...
def make_module_profile(context, profile_name, allowed_modules, force):
    ...
    try:
        ...
        if frappe.db.exists("Module Profile", profile_name) and not force:
            click.echo(
                f"Module Profile '{profile_name}' already exists. Use --force to overwrite.")
            return
...
def make_role_profile(context, profile_name, roles, force):
    ...
    try:
        ...
        if frappe.db.exists("Role Profile", profile_name) and not force:
            click.echo(
                f"Role Profile '{profile_name}' already exists. Use --force to overwrite.")
            return
