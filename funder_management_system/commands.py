...
@click.option(
	"--interactive", "-i", is_flag=True, default=True, help="Interactive mode to set up email account"
)
@pass_context
...
def _interactive_email_account_setup():
	...
	email_id = click.prompt(click.style("Email Address", fg="cyan", bold=True), type=str).strip().lower()
	...
	service_choice = click.prompt(
		click.style("Select service", fg="cyan"), type=click.IntRange(1, len(service_options)), default=1
	)
	...
	auth_choice = click.prompt(
		click.style("Select authentication method", fg="cyan"), type=click.IntRange(1, 2), default=1
	)
	...
	if auth_method == "Basic":
		...
		password = click.prompt(click.style(password_prompt, fg="yellow"), type=str, hide_input=True)
	...
	enable_incoming = click.confirm(
		click.style("Enable incoming emails (receiving)?", fg="cyan"), default=False
	)

	enable_outgoing = click.confirm(click.style("Enable outgoing emails (sending)?", fg="cyan"), default=True)
	...
	if enable_incoming:
		default_incoming = click.confirm(
			click.style("Set as default incoming account?", fg="yellow"), default=False
		)

	if enable_outgoing:
		default_outgoing = click.confirm(
			click.style("Set as default outgoing account?", fg="yellow"), default=False
		)
	...
	if not service:  # Custom service
		...
		if enable_incoming:
			use_imap = click.confirm(
				click.style("Use IMAP (recommended over POP3)?", fg="cyan"), default=True
			)

			email_server = click.prompt(
				click.style("Incoming server (e.g., imap.gmail.com)", fg="cyan"), type=str
			).strip()
			...
			use_ssl = click.confirm(click.style("Use SSL for incoming?", fg="cyan"), default=True)

		if enable_outgoing:
			smtp_server = click.prompt(
				click.style("Outgoing server (e.g., smtp.gmail.com)", fg="cyan"), type=str
			).strip()

			smtp_port = click.prompt(
				click.style("Outgoing port (587 for TLS, 465 for SSL)", fg="cyan"), type=int, default=587
			)

			use_tls = click.confirm(click.style("Use TLS for outgoing?", fg="cyan"), default=True)
	...
	track_email_status = click.confirm(
		click.style("Track email status (opens, clicks)?", fg="yellow"), default=True
	)

	add_signature = click.confirm(click.style("Add email signature?", fg="yellow"), default=False)
	...
	click.echo(
		f"Incoming Enabled: {click.style('Yes' if enable_incoming else 'No', fg='green' if enable_incoming else 'red')}"
	)
	click.echo(
		f"Outgoing Enabled: {click.style('Yes' if enable_outgoing else 'No', fg='green' if enable_outgoing else 'red')}"
	)
	...
	if click.confirm("\nCreate this email account?", default=True):
		try:
			...
			if enable_incoming and use_imap:
				email_account_doc.append(
					"imap_folder", {"folder_name": "INBOX", "append_to": "", "use_for_syncing": 1}
				)
			...
			click.echo(
				f"You can view it at: {click.style(f'/app/email-account/{email_account_doc.name}', fg='blue')}"
			)
...
def list_email_accounts(context, limit, format):
	...
	try:
		...
		if format == "json":
			...
			click.echo("=" * 120)
			click.echo(
				f"{'Account Name':<20} {'Email':<30} {'Service':<15} {'In':<3} {'Out':<3} {'Def In':<6} {'Def Out':<7} {'Created':<12}"
			)
			click.echo("-" * 120)
			...
			for acc in email_accounts:
				...
				click.echo(
					f"{acc.email_account_name[:19]:<20} {acc.email_id[:29]:<30} {(acc.service or 'Custom')[:14]:<15} {in_enabled:<3} {out_enabled:<3} {def_in:<6} {def_out:<7} {created_date:<12}"
				)
...
def create_user(context):
	...
	try:
		...
		# Get user details
		first_name = click.prompt(click.style("First Name", fg="cyan", bold=True), type=str).strip()

		last_name = click.prompt(click.style("Last Name", fg="cyan", bold=True), type=str).strip()

		email = click.prompt(click.style("Email", fg="cyan", bold=True), type=str).strip().lower()
		...
		role_choice = click.prompt(
			click.style("\nSelect role", fg="cyan"), type=click.IntRange(1, len(roles)), default=1
		)
...
@click.option(
	"--service",
	type=click.Choice(["GMail", "Outlook.com", "Yahoo Mail", "Custom"]),
	default="GMail",
...
@click.option(
	"--default-incoming/--no-default-incoming", default=False, help="Set as default incoming account"
)
@click.option(
	"--default-outgoing/--no-default-outgoing", default=False, help="Set as default outgoing account"
)
...
def setup_email_account_cli(
	...
	try:
		...
		if email_signature:
			doc_data.update({"add_signature": 1, "signature": email_signature})

		# Add incoming settings
		if incoming:
			if service == "GMail":
				doc_data.update({"email_server": "imap.gmail.com", "incoming_port": "993", "use_ssl": True})
			elif service == "Outlook.com":
				doc_data.update(
					{"email_server": "outlook.office365.com", "incoming_port": "993", "use_ssl": True}
				)
			elif service == "Yahoo Mail":
				doc_data.update(
					{"email_server": "imap.mail.yahoo.com", "incoming_port": "993", "use_ssl": True}
				)
			elif service == "Custom":
				if not imap_server:
		...
		if outgoing:
			if service == "GMail":
				doc_data.update({"smtp_server": "smtp.gmail.com", "smtp_port": "587", "use_tls": True})
			elif service == "Outlook.com":
				doc_data.update({"smtp_server": "smtp-mail.outlook.com", "smtp_port": "587", "use_tls": True})
			elif service == "Yahoo Mail":
				doc_data.update({"smtp_server": "smtp.mail.yahoo.com", "smtp_port": "587", "use_tls": True})
			elif service == "Custom":
				if not smtp_server:
...
@click.option(
	"--send-welcome-email/--no-welcome-email", default=True, help="Send welcome email with credentials"
)
@pass_context
...
def list_users(context, limit, format):
	...
	try:
		...
		for user in users:
			roles = frappe.get_all("Has Role", fields=["role"], filters={"parent": user.name}, pluck="role")
		...
		if format == "json":
			...
			for user in users:
				...
				click.echo(
					f"Status: {click.style(user.status, fg='blue' if user.status == 'Active' else 'red')}"
				)
				click.echo("-" * 40)
...
def make_module_profile(context, profile_name, allowed_modules, force):
	...
	try:
		...
		if frappe.db.exists("Module Profile", profile_name):
			...
			doc = frappe.get_doc(
				{"doctype": "Module Profile", "module_profile_name": profile_name, "block_modules": []}
			)
...
@click.command("auto-setup")
@click.option("--email", default="admin1@example.com", help="Admin email")
@click.option("--password", default="admin", help="Admin password")
@click.option("--company", default="My Company", help="Company name")
@click.option("--country", default="India", help="Country")
@click.option("--timezone", default="Asia/Kolkata", help="Timezone")
@click.option("--currency", default="INR", help="Currency")
@pass_context
def auto_setup_site(context, email, password, company, country, timezone, currency):
	"""
	Automatically complete the setup wizard for a site.

	This command will:
	1. Complete the setup wizard with provided/default values
	2. Set the home page to desktop
	3. Mark setup as complete

	Usage:
	    bench --site <site_name> auto-setup
	    bench --site <site_name> auto-setup --email admin@mycompany.com --password mypassword
	"""
	...
	try:
		...
		if frappe.is_setup_complete():
			click.echo("✅ Setup is already complete. Skipping auto-setup.")
			return

		click.echo(f"Starting automatic setup for site: {site}")
		...
		default_setup_data = {
			...
			"company_abbr": company[:2].upper(),
			"setup_complete": 1,
			"is_fundraising_admin": 1,
		}

		# Complete the setup wizard
		setup_complete(default_setup_data)

		# Set the home page to desktop instead of setup-wizard
		frappe.db.set_default("desktop:home_page", "desktop")

		# Commit the changes
		frappe.db.commit()

		click.echo("✅ Automatic setup completed successfully!")
		...
		click.echo(f"❌ Error during automatic setup: {str(e)}")
		frappe.db.rollback()
		raise
	finally:
		frappe.destroy()
...
def make_role_profile(context, profile_name, roles, force):
	...
	try:
		...
		if frappe.db.exists("Role Profile", profile_name):
			...
			doc = frappe.get_doc({"doctype": "Role Profile", "role_profile": profile_name, "roles": []})
