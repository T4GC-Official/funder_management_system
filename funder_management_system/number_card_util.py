...
def get_total_leads_by_finanacial_year(financial_year=None):
	...
	total_leads = frappe.db.count("Organisation Lead", {"financial_year_of_reachout": financial_year})

	return {"value": total_leads, "label": "Total Leads", "fieldtype": "Int"}
...
def get_conversion_rate_by_fy(financial_year=None):
	...
	total = frappe.db.count("Organisation Lead", {"financial_year_of_reachout": financial_year})

	confirmed = frappe.db.count(
		"Organisation Lead", {"financial_year_of_reachout": financial_year, "lead_stage": "Confirmed Lead"}
	)

	rate = (confirmed / total * 100) if total else 0

	return {"label": "Conversion Rate", "value": f"{rate:.2f}", "fieldtype": "Percent"}
...
def get_churn_rate_fy(financial_year=None):
	...
	churn_leads = frappe.db.count(
		"Organisation Lead",
		{"financial_year_of_reachout": financial_year, "lead_stage": ["in", ["Cold Lead", "Dropped Lead"]]},
	)

	churn_rate = (churn_leads / total * 100) if total else 0

	return {"value": f"{churn_rate:.2f}", "label": "Churn Rate", "fieldtype": "Percent"}
...
def get_total_active_donors_current_fy():
	...
	return {"value": total, "label": "Active Donors", "fieldtype": "Int"}
...
def get_total_active_grant_agreements_current_fy():
	...
	return {"value": total, "label": "Active Grant Agreements", "fieldtype": "Int"}
