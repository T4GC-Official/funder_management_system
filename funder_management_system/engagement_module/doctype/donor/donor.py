...
from frappe.utils import add_days, getdate, today  # type: ignore
...
def send_engagement_checklist_item_reminders():
	...
	for item in due_items:
		...
		frappe.db.set_value(
			"Engagement Checklist", item["item_name"], "next_reminder_date", next_reminder_date
		)
...
def calculate_next_reminder_date(current_date, frequency):
	...
	frappe.logger().info(
		f"Calculated next_reminder_date: {next_reminder_date} for frequency: {frequency} (Current Date: {current_date})"
	)
