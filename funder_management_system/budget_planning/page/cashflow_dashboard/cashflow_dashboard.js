frappe.pages['cashflow-dashboard'].on_page_load = function(wrapper) {
	...
	page.set_primary_action('Go to Budget Plan', () => {
		frappe.set_route('List', 'Budget Plan');
	});
	...
	{
		frappe.call({
			method: "funder_management_system.budget_planning.page.cashflow_dashboard.cashflow_dashboard.get_budget_cards",
			args: { financial_years: JSON.stringify(financial_years) },
			callback: function (r) {
				if (r.message) {
					const html = frappe.render_template("cashflow_dashboard", { cards: r.message });
		...
		const chartConfigs = [
			{
				...
				method: "funder_management_system.budget_planning.page.cashflow_dashboard.cashflow_dashboard.get_budget_category_wise",
				chartType: "bar",
				chartColors: ['#147878']
			},
			{
				chartTitle: "Yearly Budget Plan Distributed Sub Category Wise",
				method: "funder_management_system.budget_planning.page.cashflow_dashboard.cashflow_dashboard.get_budget_sub_category_wise",
				chartType: "bar",
				chartColors: ['#B1380B']

			},
			{
				chartTitle: "Yearly Budget Plan Utilisation",
				method: "funder_management_system.budget_planning.page.cashflow_dashboard.cashflow_dashboard.get_budget_level_utilisation",
				chartType: "bar",
				chartColors: ['#3D7317', '#ff6f61']

			},
			{
				chartTitle: "Yearly Budget Plan Utilisation by Category",
				method: "funder_management_system.budget_planning.page.cashflow_dashboard.cashflow_dashboard.get_budget_category_wise_utilisation",
				chartType: "bar",
				chartColors: ['#707070', '#0066CC']
			}
		]
