...
def get(
    ...
    if not chart:
        ...
        chart_name = "default_chart"

    labels, allocated_values, utilized_values, overutilization_percentages = [], [], [], []
    ...
    if not result:
        frappe.log_error("No results returned for the SQL query.", "SQL Query Result Error")

    for row in result:
        category = row["category"]
        allocated = float(row["total_allocated_amount"])
        utilized = float(row["total_utilized_amount"])
        percent = round(((utilized - allocated) / allocated) * 100, 2)

        labels.append(f"{category} ({percent}% over)")
        allocated_values.append(allocated)
        utilized_values.append(utilized)
        overutilization_percentages.append(percent)
