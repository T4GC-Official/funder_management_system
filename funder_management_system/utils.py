...
def add_permissions(role, doctype, permissions, permlevel=0):
	...
	if frappe.get_all("Custom DocPerm", filters=filters):
		print(f"! Permissions already exist for role '{role}' on '{doctype}' at level {permlevel}. Skipping.")
		return
	...
	doc.insert(ignore_permissions=True)
	print(f"Granted {list(permissions.keys())} on '{doctype}' to role '{role}' at level {permlevel}")
...
def share_custom_number_cards_with_everyone():
	cards = [
		"Churn Rate In Current Financial Year",
	...
	for card in cards:
		try:
			set_permission(
				doctype="Number Card", name=card, user=None, permission_to="read", value=1, everyone=1
			)
			frappe.db.commit()
...
def update_settings():
	...
	try:
		...
		if not system_settings.language:
			system_settings.language = frappe.defaults.get_global_default("language") or "en"
		if not system_settings.time_zone:
			system_settings.time_zone = frappe.defaults.get_global_default("time_zone") or "Asia/Kolkata"
...
def total_conversion(total: float) -> dict:
	...
	"""
	if total is None:
		return {"value": "0.00", "fieldtype": "Data"}
	if total >= 10000000:
	...
	return {"value": formatted_total, "fieldtype": "Data"}
...
def get_module_profile(module_profile: str):
	...
	allowed_modules = frappe.get_all(
		"Module Def",
		filters={"app_name": "funder_management_system", "name": ["not in", blocked_modules]},
		pluck="name",
	)
...
def get_fms_modules():
	...
	modules = frappe.get_all(
		"Module Def", filters={"app_name": "funder_management_system"}, pluck="module_name"
	)
	return modules
...
def limit_storage_quota(doc, method):
	...
	if total_file_storage + doc.file_size > max_storage_bytes:
		frappe.throw(
			_(
				"You have reached your storage limit of {0} MB. Please delete unused files or upgrade your plan."
			).format(max_storage_quota)
		)
