...
from funder_management_system.engagement_module.doctype.grant_agreement.grant_agreement import (
	update_total_grant_amount_utilised,
)
...
loger_update_total = frappe.logger("total_grant_amount", allow_site=True, file_count=5)
...
class ExpenseItem(Document):
	def on_cancel(self):
		try:
			...
			for row in utilisation_record.utilisation_child_table:
				...
				if (
					row.utilisation_status == "Submitted"
					or row.utilisation_status == "Amended"
					or row.utilisation_status == "Draft"
				) and row.expenditure_record_name == self.name:
	...
	def on_trash(self):
		try:
			user = frappe.session.user
			logger.info(f"{user} requested to delete to delete expense item record: {self.name}")
			utilisation_record = frappe.get_doc("Utilisation Record", self.urn)
			for row in utilisation_record.utilisation_child_table:
...
def create_utilisation_entries(document_name):
	try:
		...
		if count > 0:
			...
			return {
				"status": True,
				"message": f"Successfully created {count} utilisation records for {document_name}",
			}
			...
			return {
				"status": False,
				"message": "No new Expense Items Entries were created (Already Processed)",
			}
	except Exception as e:
...
def update_expenditure_for_utilisation_entries(utilisation_entries):
	for utilisation_entry in utilisation_entries:
		...
		try:
			update_grant_expenditure(
				utilisation_doc.grant_agreement,
				utilisation_doc.grant_agreement_tranche,
				utilisation_doc.utilised_amount,
			)
		except Exception:
...
def submit_record(document_name):
	try:
		...
		if not document:
			frappe.throw("Expense Item not found", exc=frappe.DoesNotExistError)
		update_grant_expenditure(
			document.grant_agreement,
			document.grant_agreement_tranche,
			document.utilised_amount,
			increase=True,
		)
		user = frappe.session.user
