frappe.pages['fms-permission-manag'].on_page_load = (wrapper) => {
	let page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __("FMS Role Permissions Manager"),
		card_layout: true,
		single_column: true,
	});
...
frappe.pages['fms-permission-manag'].refresh = function (wrapper) {
...
frappe.PermissionEngine = class PermissionEngine {
	...
	make() {
		...
		frappe.call({
			method: "funder_management_system.funder_management_system.page.fms_permission_manag.fms_permission_manag.get_roles_and_doctypes",
		}).then((res) => {
	...
	setup_page() {
		...
		this.doctype_select = this.wrapper.page.add_field({
			...
			filters: {
				...
				istable: 0,
				issingle: 0,
				name: ["not in", ["User Creation"]],
			},
			change: function () {
				frappe.set_route("fms-permission-manag", this.get_value());
			},
		});
	
		// Role Link field, dynamically filtered
		this.role_select = this.wrapper.page.add_field({
			...
			get_query: () => {
				return {
					query: "funder_management_system.funder_management_system.page.fms_permission_manag.fms_permission_manag.get_fms_roles"
				};
			},
			change: () => this.refresh()
		});
	...
	get_standard_permissions(callback) {
		...
		if (doctype) {
			return frappe.call({
				module: "funder_management_system.funder_management_system",
				page: "fms_permission_manag",
