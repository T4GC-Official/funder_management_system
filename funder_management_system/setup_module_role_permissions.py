...
from .utils import add_permissions, create_roles_if_missing, setup_fms_dashboard_permissions
...
PERMISSION_SETS = {
	...
	"view_access": {"read": 1, "report": 1, "export": 1, "print": 1, "share": 1, "email": 1},
	"select_read_write": {
		"read": 1,
		"select": 1,
		"write": 1,
	...
	"report_only": {"report": 1},
	"perm_level_1": {"read": 1, "write": 1},
	"read_only": {"read": 1},
	"todo_access": {
		"read": 1,
		"write": 1,
...
def setup_budget_allocation_module_roles():
	...
	try:
		for role, doctype, permission_key, permlevel in role_doctypes_permissions_level_mapping:
			add_permissions(role, doctype, PERMISSION_SETS[permission_key], permlevel)
...
def setup_donor_acquisition_module_roles():
	...
	role_doctypes_permissions_level_mapping = [
		...
		("Donor Acquisition Full Access", "Preferred Means of Communication", "full_access", 0),
		...
		("Donor Acquisition View Access", "Preferred Means of Communication", "view_access", 0),
...
def setup_donor_engagement_module_roles():
	...
	role_doctypes_permissions_level_mapping = [
		# full access
		("Donor Engagement Full Access", "Engagement Checklist Master", "full_access", 0),
		...
		# only view access
		("Donor Engagement View Access", "Engagement Checklist Master", "view_access", 0),
...
def setup_utilisation_module_roles():
	...
	role_doctypes_permissions_level_mapping = [
		...
		("Utilisation Module Full Access", "Expense Item Child Table", "full_access", 0),
		...
		("Utilisation Module Full Access", "Engagement Checklist Master", "read_only", 0),
		("Utilisation Module View Access", "Expense Item", "view_access", 0),
		("Utilisation Module View Access", "Expense Item Child Table", "view_access", 0),
		...
		("Utilisation Module View Access", "Engagement Checklist Master", "read_only", 0),
	]
...
def add_dashboard_list_entries():
	dashboard_pages = [
		{"name": "Fundraising Dashboard", "page": "fundraising-dashboard"},
		{"name": "Donor Acquisition Dashboard", "page": "donor-acquisition-ma"},
		{"name": "Cashflow Dashboard", "page": "cashflow-dashboard"},
	]

	for entry in dashboard_pages:
		if not frappe.db.exists("Dashboard List", entry["name"]):
			doc = frappe.get_doc(
				{"doctype": "Dashboard List", "name": entry["name"], "dashboard": entry["page"]}
			)
